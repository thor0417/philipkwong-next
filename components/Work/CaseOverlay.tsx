...
export function CaseOverlay({ caseId, onClose }: CaseOverlayProps) {
  ...
  const openOverlay = useCallback(() => {
    ...
    gsap.set([topBarRef.current, contentRef.current, footerRef.current], { opacity: 0 });
    ...
    activeTlRef.current = gsap.timeline()
      ...
      .to(
        [topBarRef.current, contentRef.current, footerRef.current],
  ...
  const closeOverlay = useCallback(() => {
    ...
    activeTlRef.current = gsap.timeline()
      .to(
        [footerRef.current, contentRef.current, topBarRef.current],
  ...
  /* Open/close based on caseId prop */
  useEffect(() => {
    if (caseId) {
      openOverlay();
    } else {
